...
function make_css_cursor(name, coords, fallback) {
	return `url(/win98-web/apps/paint/images/cursors/${name}.png) ${coords.join(" ")}, ${fallback}`;
}
...
/**
 * @param {string} file_name  name of an image file in the /win98-web/apps/paint/help/ folder, including extension
 * @returns {HTMLImageElement}  an image element
 */
function get_help_folder_icon(file_name) {
	const icon_img = new Image();
	icon_img.src = `/win98-web/apps/paint/help/${file_name}`;
	return icon_img;
}
...
function get_icon_for_tools(tools) {
	...
	Promise.all(tools.map((tool) => load_image_simple(`/win98-web/apps/paint/help/${tool.help_icon}`)))
		.then((icons) => {
