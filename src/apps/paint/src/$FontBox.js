...
const eachFont = async (callback, afterAllCallback) => {
  ...
  if (window.queryLocalFonts) {
    ...
    if (availableFonts.length === 0) {
      console.log(
        "queryLocalFonts returned no fonts; falling back to FontDetective.",
      );
      localFontAccessUnavailable();
      return;
    }
...
function $FontBox() {
  ...
  const $family =
    /** @type {JQuery<HTMLSelectElement & { type: "select-one" }>} */ (
      $(E("select")).attr({
  ...
  const $bold = $Toggle(
    ...
    "bold",
    "Bold",
    localize("Sets or clears the text bold attribute."),
  );
  const $italic = $Toggle(
    1,
    "italic",
    "Italic",
    localize("Sets or clears the text italic attribute."),
  );
  const $underline = $Toggle(
    2,
    "underline",
    "Underline",
    localize("Sets or clears the text underline attribute."),
  );
  const $vertical = $Toggle(
    3,
    "vertical",
    "Vertical Writing Mode",
    localize("Only a Far East font can be used for vertical editing."),
  );
  ...
  function $Toggle(xi, thing, label, description) {
    ...
    $button.on("click", () => {
      ...
      $button.attr(
        "aria-pressed",
        $button.hasClass("selected") ? "true" : "false",
      );
      update_font();
    });
